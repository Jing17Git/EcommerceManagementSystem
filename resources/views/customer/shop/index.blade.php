...
<section class="bg-[#1c1e38] border-b border-orange-500/10 px-6 md:px-14 py-16 relative overflow-hidden">
    ...
    <p class="text-gray-400 text-base max-w-xl leading-relaxed">Browse unique items from local artisans.</p>
</section>
...
<section class="px-6 md:px-14 py-12 max-w-6xl mx-auto">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 mb-14">
        <a href="{{ route('shop') }}" class="bg-[#1c1e38] border {{ empty($selectedCategory) ? 'border-orange-500/70' : 'border-white/5' }} rounded-2xl p-5 text-center hover:border-orange-500/30 hover:-translate-y-1 transition-all duration-200">
            <div class="text-2xl mb-3">ALL</div>
            <p class="text-white text-sm font-semibold">All Categories</p>
        </a>
        @forelse(($categories ?? collect()) as $category)
            <a href="{{ route('shop', ['category' => $category->slug]) }}" class="bg-[#1c1e38] border {{ isset($selectedCategory) && $selectedCategory?->id === $category->id ? 'border-orange-500/70' : 'border-white/5' }} rounded-2xl p-5 text-center hover:border-orange-500/30 hover:-translate-y-1 transition-all duration-200">
                <div class="text-2xl mb-3">#</div>
                <p class="text-white text-sm font-semibold">{{ $category->name }}</p>
            </a>
        @empty
            <div class="col-span-6 bg-[#1c1e38] border border-white/5 rounded-2xl p-5 text-center text-gray-400 text-sm">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            @if(isset($selectedCategory) && $selectedCategory)
                <p class="text-sm text-orange-400">Filtered by: {{ $selectedCategory->name }}</p>
            @endif
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        ...
        <div class="bg-[#1c1e38] border border-white/5 rounded-2xl overflow-hidden hover:border-orange-500/30 hover:-translate-y-1 hover:shadow-xl transition-all duration-200">
            <div class="h-44 bg-[#12132a] overflow-hidden">
                ...
                @else
                    <div class="w-full h-full flex items-center justify-center text-4xl">P</div>
                @endif
            </div>
            <div class="p-5">
                ...
                <div class="flex items-center justify-between">
                    <span class="text-orange-400 font-bold">PHP {{ number_format((float)$product->price, 2) }}</span>
                    @auth
    ...
    <div class="bg-[#1c1e38] border border-white/5 rounded-2xl p-16 text-center">
        ...
        <h3 class="text-white font-semibold mb-2">No products in this category</h3>
        <p class="text-gray-400 text-sm">Try another category or check back soon.</p>
    </div>
    @endif
</section>
@endsection
