...
<style>
  ...
  .form-card-head-left { display: flex; align-items: center; gap: .75rem; }
  .form-card-icon {
...
<div class="form-card">
  ...
  <div class="form-card-head">
    <div class="form-card-head-left">
      ...
      <div>
        <p class="form-card-title">{{ $category->name }}</p>
        <p class="form-card-sub">ID #{{ $category->id }} · Last updated {{ $category->updated_at->diffForHumans() }}</p>
      </div>
    </div>
  ...
  <form action="{{ route('admin.categories.update', $category) }}" method="POST">
    ...
    <div class="form-card-body">
      ...
      <div class="f-group">
        ...
        <div class="f-hint">Edit the slug manually if needed.</div>
        @error('slug')
          <div class="f-error">
            <svg width="11" height="11" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
            {{ $message }}
    ...
    <div class="form-card-foot">
      ...
      <form action="{{ route('admin.categories.destroy', $category) }}" method="POST"
            onsubmit="return confirm('Are you sure you want to delete {{ addslashes($category->name) }}?')">
        @csrf
        @method('DELETE')
