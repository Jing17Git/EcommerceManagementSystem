...
class BuyerController extends Controller
{
    ...
    /**
     * Submit seller application with improved architecture.
     */
    public function submitSellerApplication(SellerApplicationRequest $request, SellerApplicationService $service)
    {
        $user = Auth::user();
        ...
        if ($user->isSeller()) {
            return redirect()->route('buyer.dashboard')
                ->with('error', 'You are already a seller.');
        }
        ...
        try {
            ...
            // Store ID card document
            $service->storeDocument(
                $application,
                $request->file('id_card'),
                SellerApplicationDocument::TYPE_ID_CARD
            );
            
            // Auto-validate and process application
            ...
            if ($application->isApproved()) {
                return redirect()->route('buyer.dashboard')
                    ->with('success', 'Your seller application has been automatically approved! You can now switch to seller mode.');
            } else {
                return redirect()->route('buyer.dashboard')
                    ->with('error', 'Your seller application was rejected: ' . $application->rejection_reason);
            }
    ...
    public function requestReturn(Order $order, Request $request)
    {
        ...
        if ($order->status !== 'delivered') {
            return back()->with('error', 'Only delivered orders can be requested for return.');
        }
        ...
        ReturnRequest::create([
            ...
            'reason' => $validated['reason'] ?? 'Buyer requested return.',
            'status' => 'requested',
        ]);
        ...
        return Order::where('user_id', $userId)
            ...
            ->map(function (Order $order) {
                ...
                if ($order->returnRequest) {
                    $message = match ($order->returnRequest->status) {
                        'requested' => "Return request for {$orderCode} is awaiting seller review.",
                        'approved' => "Return request for {$orderCode} has been approved.",
                        'rejected' => "Return request for {$orderCode} was rejected by seller.",
                        'refunded' => "Return request for {$orderCode} has been refunded.",
                        default => "Return update for {$orderCode}.",
                    };
                } else {
                ...
                foreach ($groupedItems as $sellerId => $items) {
                    ...
                    $order->statusHistories()->create([
                        ...
                        'note' => $status === 'processing'
                            ? 'Order placed and auto-accepted by seller settings.'
                            : 'Order placed and waiting for seller confirmation.',
                        'changed_by' => $userId,
                    ]);
                }
